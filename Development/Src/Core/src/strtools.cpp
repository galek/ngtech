...
char *V_pretifynum( long long value )
{
	static char output[ NUM_PRETIFYMEM_BUFFERS ][ 32 ];
	static int  current;
...
void V_binarytohex( const byte *in, int inputbytes, char *out, int outsize )
{
	...
	for ( i = 0; i < inputbytes; i++ )
	{
		...
		V_strncat( out, doublet, outsize, -1 );
	}
}
...
void V_DefaultExtension( char *path, const char *extension, int pathStringLength )
{
	...
	V_strncat( path, extension, pathStringLength, -1 );
}
...
void V_ComposeFileName( const char *path, const char *filename, char *dest, int destSize )
{
	...
	V_strncat(dest, filename, destSize, -1);
	V_FixSlashes( dest );
}
...
void V_AppendSlash( char *pStr, int strSize )
{
	...
	{
		if ( len+1 >= strSize )
			Warning( "V_AppendSlash: ran out of space on %s.", pStr );
	...
	else
	{
		...
		else
		{
			if ( !_getcwd( pOut, outLen ) )
				Warning( "V_MakeAbsolutePath: _getcwd failed." );

			if ( pStartingDir )
			{
				V_AppendSlash( pOut, outLen );
				V_strncat(pOut, pStartingDir, outLen, -1);
			}
		}

		// Concatenate the paths.
		V_AppendSlash( pOut, outLen );
		V_strncat(pOut, pPath, outLen, -1);
	}

	if ( !V_RemoveDotSlashes( pOut ) )
		Warning( "V_MakeAbsolutePath: tried to \"..\" past the root." );
...
void V_SplitString2( const char *pString, const char **pSeparators, int nSeparators, std::vector<char*> &outStrings )
{
	//outStrings.Purge();
...
void V_SplitString( const char *pString, const char *pSeparator, std::vector<char*> &outStrings )
{
