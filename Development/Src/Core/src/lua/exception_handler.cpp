...
#include <lua/error.hpp>            // for error
#include <lua/exception_handler.hpp>  // for exception_handler_base
...
LUABIND_API void handle_exception_aux(lua_State* L)
{
    try
    {
        if (handler_chain)
			...
			throw std::runtime_error("handle_exception_aux-failed handler_chain->handle");
    }
    catch (error const&)
    {}
