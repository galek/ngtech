...
public:
	...
	// From ITrackViewNode
	void AddNode(ITrackViewNode *node, const MCHAR *name, Class_ID cid, int pos=TVNODE_APPEND){};
	void AddController(Control *c, const MCHAR *name, Class_ID cid, int pos=TVNODE_APPEND){};
	...
	const MCHAR *GetName(int i) {return podlist[i]->GetName();}
	virtual void SetName(int i,const MCHAR *name) {};
