...
//skinGetBoneNameByListID listID
//just returns the name of the bone
def_struct_primitive_debug_ok( getBoneNameByListID, skinOps, "GetBoneNameByListID");
...
check_bone_index(BonesDefMod* bmod, int index)
{
	...
	if(bmod && (NULL == bmod->BoneData[index].Node))
		throw RuntimeError(_T("Invalid Bone index in the scene: "), Integer::intern(index+1));
	return index;
}

static int 
check_bone_list_index(BonesDefMod* bmod, int index)
{
	DbgAssert(NULL != bmod);
	int ct = 0;
	...
	if (index < 0 || index >= ct) 
		throw RuntimeError(_T("Bone index out of range: "), Integer::intern(index + 1));
	int boneId = bmod->ConvertSelectedListToBoneID(index);
	if(NULL == bmod->BoneData[boneId].Node)
		throw RuntimeError(_T("Invalid Bone index in the scene: "), Integer::intern(boneId+1));
	return boneId;
}
...
static Value* GetBoneNameFromIndex(BonesDefMod* bmod, int index, int listName)
{
...
Value*
getBoneNameByListID_cf(Value** arg_list, int count)
{
	check_arg_count(getBoneNameByListID, 3, count);
	get_bonedef_mod();
	//get first mod context not sure how this will work if multiple instanced are selected
	//maybe should use COREinterface instead of local interface ????
	if ( !bmod->ip ) throw RuntimeError(GetString(IDS_PW_SKIN_NOT_SELECTED), arg_list[0]);
	...
	index = check_bone_list_index(bmod, index);

	return GetBoneNameFromIndex(bmod, index, listName);
