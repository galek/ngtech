...
#ifdef FIX_MAXX_7661
	isGlobal = isGlobal && (chMtlIndex ? chMtlIndex->IsGlobal() : true);
	isGlobal = isGlobal && mappingGlobal(chMapping);
#endif

	// no sharing at all for the output shape?
	bool isNotShared = !isGlobal || !chShape->IsShared() || (chShape->GetValueCount() >= m_count);
#ifdef FIX_MAXX_7661
	isNotShared = isNotShared || mappingNotShared(chMapping) || materialNotShared(chMtlIndex);
#endif
...
#else
		ShapeShareIndex curSSI(i, chShape, nullptr, 0, nullptr);
#endif
